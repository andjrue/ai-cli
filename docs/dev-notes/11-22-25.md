...
# 11-23-25 Update
Made a lot of progress but got stuck on the OAI response streaming issue. That took a bit longer than I expected it to. Still managed to add Anthropic, so claude is alive in the TUI and we have support for different models. 

Still need to continue the UI refactor, but keybindings are working. Now that we have those, I need to setup a help modal that shows the keybinds, what they do, and more information that points to documentation in github. A getting started guide should suffice, it's as easy as setting up the yaml file. 

Next up is adding conversation storage and chat history. This will open as a separate modal and let users pull in information from historic chats. Should be a fun add. 

While I'm doing that, I can also add the system prompt to the config file. A user could make this whatever they wanted to, mine will be pretty simple. 